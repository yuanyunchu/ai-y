...
| **输出格式** | OpenSpec `design.md` + `specs/<capability>/spec.md` |
| **成本级别** | 高（5x baseline），开启推理模式仅用于设计阶段 |
| **输入类型** | proposal.md + ai-analysis.md + 非功能需求 |
| **OpenSpec 阶段** | Propose |
| **输出位置** | `openspec/changes/<name>/design.md`<br>`openspec/changes/<name>/specs/<capability>/spec.md`<br>`project-context/02-design/<name>/`（api-spec.yaml, db-schema.sql, sentinel-rules.json） |
| **关键规则** | 必须做服务拆分合理性判断<br>数据库设计满足第三范式或说明反范式理由<br>跨服务接口必须设计幂等性和超时重试<br>分布式事务方案避开长事务 |
...
|------|-----|
| **首选模型** | DeepSeek-v4-pro |
| **备选模型** | o3-mini-high / Claude 3.7 Sonnet |
| **核心能力** | 理解深层依赖和边界条件 |
| **目标指标** | 逻辑完整性（零重大 Bug） |
| **输出格式** | Java 源码（Controller → Service → Mapper 分层） |
| **成本级别** | 高（5x baseline） |
| **输入类型** | design.md + architecture.md + 现有参考代码 |
| **OpenSpec 阶段** | Apply |
| **输出位置** | `project-context/03-core/<name>/` + 实际项目源码 |
| **关键规则** | 先输出伪代码/思考过程，确认后输出正式代码<br>禁止直接合入，需 Code Review 或对抗性审查<br>复杂度不超过 O(n log n)<br>禁止 N+1 查询<br>批量操作阈值 500 条/批 |
...
| 属性 | 值 |
|------|-----|
| **首选模型** | Ark-code-latest |
| **备选模型** | GPT-4o / Gemini 2.5 Flash |
| **核心能力** | 高速生成，快速响应 |
| **目标指标** | 吞吐量（一次生成整个模块） |
| **输出格式** | CRUD 代码、DTO/VO/Converter、单元测试、Feign 接口 |
| **成本级别** | 低（1x baseline），批量生成减少往返 |
| **输入类型** | api-spec.yaml + 数据库表结构 |
| **OpenSpec 阶段** | Apply |
| **输出位置** | `project-context/04-standard/generated/` + 实际项目源码 |
| **关键规则** | 可批量生成一个完整模块<br>人工快速 Review 即可，重点看字段映射和空指针<br>严格遵循分层规范和编码规范（见 coding-standard.md） |
...
| **成本级别** | 极低（0.1x baseline），可无限使用 |
| **输入类型** | 错误日志（需脱敏）、Stack Trace、相关代码 |
| **OpenSpec 阶段** | Apply / Archive |
| **输出位置** | `project-context/05-debug/bug-fixes.md` |
| **关键规则** | 日志脱敏后再给 AI<br>快问快答，不需要长上下文<br>使用模型最小版本追求速度 |
...
| 环节 | 模型类型 | 相对成本 | 省钱技巧 |
|------|---------|---------|---------|
| ① | 长上下文 | 10x | 先轻量提取摘要，再深度分析（降 70%） |
| ②③ | 深度推理 | 5x | 推理模式仅用于设计，代码阶段关闭深度思考 |
| ④ | 高速代码 | 1x | 批量生成，减少往返次数 |
| ⑤ | 轻量快反 | 0.1x | 无限使用，但注意日志脱敏自动化 |
...
```
project-context/00-global/coding-standard.md ──→ 所有环节均作为 system prompt
project-context/00-global/api-conventions.yaml ──→ 环节②④ 必读
project-context/00-global/architecture-baseline.md ──→ 环节①② 必读

环节① 输出 (proposal + ai-analysis)  ──→ 环节② 输入
环节② 输出 (design + specs + tasks)   ──→ 环节③④ 输入
环节③④ 输出 (源码 + UT)              ──→ 环节⑤ 输入（如遇 Bug）
